...
const CreateQuiz = () => {
    ...
    const [questions, setQuestions] = useState([{text : '', errorText : '', answers : []}]);
    ...
    const addAnswer = (questionIndex) => {
        const newQuestions = [...questions];
        newQuestions[questionIndex].answers.push({text : '', errorText : '', isCorrect : false});
        setQuestions(newQuestions);
    }
    ...
    const handleBlur = (elementId, value, validateMethod, setErrorMethod, errorMessage) => {
    ...
    const handleAnswerBlur = (questionIndex, answerIndex) => {
        ...
        if(!validateAnswer(questionIndex, answerIndex)) {
            newQuestions[questionIndex].answers[answerIndex].errorText = 'Answer is too short!';
            element.classList.add('is-invalid');
            element.classList.remove('is-valid');
        } else {
    ...
    const validateForm = () => {
        ...
        for(let i = 0; i < questions.length; i++) {
            if(questions[i].text.length === 0 || questions[i].errorText.length > 0) return false;
            for(let j = 0; j < questions[i].answers.length; j++) {
                if(questions[i].answers[j].text.length === 0 || questions[i].answers[j].errorText.length > 0) return false;
            }
        }

        // Check, if other attributes are not empty
        if(nameError || descriptionError || categoryError || imageUrlError ||
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:8000/php/insert_quiz.php', {
                method: 'POST',
                body: formData
            });
        ...
        <div className="main_container">
            ...
            <form onSubmit={handleSubmit}>
               <QuizInput
                  ...
                  handleBlur={() => handleBlur('name', name, validateName, setNameError, "Quiz name is too short!")}
                  errorMsg={nameError}
                />
                ...
                <QuizInput
                  ...
                  handleBlur={() => handleBlur('description', description, validateDescription, setDescriptionError, "Quiz description is too short!")}
                ...
                <QuizInput
                  ...
                  handleBlur={() => handleBlur('category', category, validateCategory, setCategoryError, "Quiz category is too short!")}
                ...
                <QuizInput
                  ...
                  handleBlur={() => handleBlur('image-url', imageUrl, validateImageUrl, setImageUrlError, "Invalid URL address!")}
                ...
                <div className="button-container">
                    <button type="button" className="quiz-btn" onClick={addQuestion}>Add question</button>
                    <button type="submit" className="create-quiz-btn">Create!</button>
                </div>
